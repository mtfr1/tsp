...
if inpt == 1:
    ...
    with open('euclidxmanh.csv', 'w') as f:
        f.write('tsp_func,num_verts,length,time,dist_type\n')

    n_vertex = [n for n in range(1000, 3001, 200)]
    dists = [euclid_dist, manhattan_dist]
    algorithms = [twice_around]
    algo_str = ["Twice around the tree"]
    dist_str = ["Euclides", "Manhattan"]
    for n in n_vertex:
        points = gen_points(n)
        ...
        for i in range(len(algorithms)):
            for j in range(len(dists)):
                ...
                ans = algo_str[i] + "," + str(n) + "," + str(value) + "," + str(total_time) + "," + dist_str[j] + "\n"
                
                with open('euclidxmanh.csv', 'a') as f:
                    f.write(ans)